...
public class MetricsDashboardServerTest {
  ...
  private void runWithServer(Vertx vertx, TestContext ctx, java.util.function.Consumer<Integer> body) {
    MetricRegistry registry = new MetricRegistry();
    registry.counter("proxy.http.requests.total").inc(7);
  ...
  public void serveDashboardHtmlOnRoot(TestContext ctx) {
    ...
    runWithServer(vertx, ctx, port -> {
      HttpClient client = vertx.createHttpClient();
      client
          .request(new RequestOptions().setMethod(HttpMethod.GET).setHost("127.0.0.1").setPort(port).setURI("/"))
          .compose(req -> req.send())
          .onFailure(ctx::fail)
          .onSuccess(resp -> {
            ctx.assertEquals(200, resp.statusCode());
            ctx.assertTrue(resp.getHeader("content-type").toLowerCase().contains("text/html"));
            resp.body()
                .onFailure(ctx::fail)
                .onSuccess(body -> {
                  String html = body.toString();
                  ctx.assertTrue(
                      html.contains("echarts.min.js"), "HTML should reference ECharts CDN");
                  ctx.assertTrue(
  ...
  public void serveJsonOnApiMetrics(TestContext ctx) {
    ...
    runWithServer(vertx, ctx, port -> {
      HttpClient client = vertx.createHttpClient();
      client
          .request(new RequestOptions().setMethod(HttpMethod.GET).setHost("127.0.0.1").setPort(port).setURI("/api/metrics"))
          .compose(req -> req.send())
          .onFailure(ctx::fail)
          .onSuccess(resp -> {
            ctx.assertEquals(200, resp.statusCode());
            ctx.assertTrue(resp.getHeader("content-type").toLowerCase().contains("application/json"));
            resp.body()
                .onFailure(ctx::fail)
                .onSuccess(body -> {
                  ...
                  ctx.assertEquals(
                      7L,
                      (long) json.getJsonObject("counters").getLong("proxy.http.requests.total"));
                  ctx.assertNotNull(
                      json.getJsonObject("meters").getJsonObject("proxy.http.bytes.up"));
                  done.complete();
                });
          });
    });
  ...
  public void unknownPathReturns404(TestContext ctx) {
    ...
    runWithServer(vertx, ctx, port -> {
      HttpClient client = vertx.createHttpClient();
      client
          .request(new RequestOptions().setMethod(HttpMethod.GET).setHost("127.0.0.1").setPort(port).setURI("/anything"))
          .compose(req -> req.send())
          .onFailure(ctx::fail)
          .onSuccess(resp -> {
